...
var Dialog = function () {
	...
	if(!this.ID)
        this.ID = 1;//Dialog._Array.length + "";
...
Dialog.prototype.attachBehaviors = function () {
    if (this.Drag && Drag) Drag.init($id("_Draghandle_" + this.ID), $id("_DialogDiv_" + this.ID)); //注册拖拽方法
    if (!isIE && this.URL) { //非ie浏览器下在拖拽时用一个层遮住iframe，以免光标移入iframe失去拖拽响应
        var self = this;
        $id("_DialogDiv_" + this.ID).onDragStart = function () {
            $id("_Covering_" + self.ID).style.display = ""
        }
        $id("_DialogDiv_" + this.ID).onDragEnd = function () {
            $id("_Covering_" + self.ID).style.display = "none"
        }
    }
};
...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      <td width="13" height="33" style=""><div style="width: 13px;"></div></td>\
      <td height="43" style="background-color: #039b46;"><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;margin-top:7px;"><span style="font-size:16px;"  id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);"  style="margin-top:2px; position: relative; cursor: pointer; float: right; color:#fff;font-size:20px;' + (this.ShowCloseButton ? "" : "display:none;") + '"><img style="margin-top:0px;" src="'+IMAGESPATH+'dialog_closebtn.gif"/></div></td>\
      <td width="13" height="33" style=""><div style="width: 13px;"><a id="_forTab_' + this.ID + '" href="#;"></a></div></td>\
    </tr>\
    <tr valign="top">\
      <td width="13" style="style="bgcolor:#fff;"></td>\
	...
	' + (function (obj) {
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #fff; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
    ...
    <tr>\
      <td width="13" height="13" style=""></td>\
      <td style=""></td>\
      <td width="13" height="13" style=""><a onfocus=\'$id("_forTab_' + this.ID + '").focus();\' href="#;"></a></td>\
    </tr>\
  </table>\
</div>\
'
    var div = $id("_DialogDiv_" + this.ID);
    if (!div) {
    ...
    if (this.InvokeElementId) {
        ...
        if (isIE) {
            ...
            element.outerHTML = "";
            $id("_Covering_" + this.ID).parentNode.appendChild(fragment)
        } else {
            $id("_Covering_" + this.ID).parentNode.appendChild(element)
        }
    }
    this.parentWindow = window;
    if (this.URL) {
        if ($id("_DialogFrame_" + this.ID)) {
            this.innerFrame = $id("_DialogFrame_" + this.ID);
        };
        var self = this;
        innerFrameOnload = function () {
            try {
                ...
                if (!self.Title && self.innerDoc && self.innerDoc.title) {
                    if (self.innerDoc.title) $id("_Title_" + self.ID).innerHTML = self.innerDoc.title;
                };
            } catch(e) {
    ...
    $id("_DialogDiv_" + this.ID).dialogId = this.ID;
    $id("_DialogDiv_" + this.ID).dialogInstance = this;
    this.attachBehaviors();
    this.okButton = $id("_ButtonOK_" + this.ID);
    this.cancelButton = $id("_ButtonCancel_" + this.ID);
	div=null;
};
Dialog.prototype.setSize = function (w, h) {
    if (w && +w > 20) {
        this.Width = +w;
        $id("_DialogTable_" + this.ID).width = this.Width + 26;
        $id("_Container_" + this.ID).style.width = this.Width + "px";
    }
    if (h && +h > 10) {
        this.Height = +h;
        $id("_Container_" + this.ID).style.height = this.Height + "px";
    }
    this.setPosition();
};
Dialog.prototype.show = function () {
    ...
    if (Dialog._Array.length > 0) {
        this.zindex = thisdialogDiv.style.zIndex = Dialog._Array[Dialog._Array.length - 1].zindex + 2;
    } else {
    ...
    if (this.Modal) {
        bgdiv.style.zIndex = Dialog._Array[Dialog._Array.length - 1].zindex - 1;
        Dialog.setBgDivSize();
		if(bgdiv.style.display == "none"){
			if(this.Animator){
				var bgMask=$id("_DialogBGMask");
...
Dialog.prototype.close = function () {
    ...
    if (this == Dialog._Array[Dialog._Array.length - 1]) {
        var isTopDialog = Dialog._Array.pop();
    } else {
        Dialog._Array.remove(this)
    }
    if (this.InvokeElementId) {
        var innerElement = $id(this.InvokeElementId);
    ...
    if (Dialog._Array.length > 0) {
        if (this.Modal && isTopDialog) Dialog.bgDiv.style.zIndex = Dialog._Array[Dialog._Array.length - 1].zindex - 1;
    } else {
    ...
    if (isIE) {
		...
		$id("_DialogDiv_" + this.ID).onDragStart=null;
		$id("_DialogDiv_" + this.ID).onDragEnd=null;
		$id("_Draghandle_" + this.ID).onmousedown=null;
		$id("_Draghandle_" + this.ID).root=null;
		/**end释放引用**/
        thisdialogDiv.outerHTML = "";
		CollectGarbage();
    } else {
        var RycDiv = $id("_RycDiv");
        if (!RycDiv) {
...
Dialog.getInstance = function (id) {
    var dialogDiv = $id("_DialogDiv_" + id);
...
Dialog.prototype.addButton = function (id, txt, func) {
    $id("_ButtonRow_" + this.ID).style.display = "";
    ...
    button.value = txt;
    button.onclick = func;
    var input0 = $id("_DialogButtons_" + this.ID).getElementsByTagName("INPUT")[0];
...
Dialog.prototype.getDialogDiv = function () {
	var dialogDiv=$id("_DialogDiv_" + this.ID)
...
function showImage (src) {
	...
	if(img){
		...
		diag.InnerHtml="<img src= '"+src+"' />";
		diag.OKEvent = function(){diag.close();};//点击确定后调用的方法
		diag.show();
	}
}
function showMsg (msg,func){
	Dialog.alert("<br/><span style='color:#039b46'>"+msg+"</span><br/>",func);
}
